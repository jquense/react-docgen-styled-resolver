...
function setPropDescriptor(
  ...
  if (t.ObjectTypeSpreadProperty.check(path.node)) {
    ...
    const resolvedPath = resolveToValue(name, importer);
...
export function createHandler({ moduleName }: any = {}) {
  function addComposes(tagPath: any, doc: Documentation, importer: any) {
    ...
    if (isSimpleStyled(tagPath)) {
      ...
      composes = tagPath.get("callee").get("object").get("arguments").get(0);
    }

    if (!composes) return;

    if (t.Identifier.check(composes.node)) {
      const mod = resolveToModule(composes, importer);
      if (mod) {
        doc.addComposes(mod);
      }
    }
  ...
  return (documentation: Documentation, path: any, importer) => {
    ...
    addComposes(tagPath, documentation, importer);

    const typePath = path.get("typeParameters").get("params").get(0);
